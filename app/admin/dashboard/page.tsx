...
import { Briefcase, MessageSquare, FileText, Star, ArrowUpRight, Clock, Mail, Users, TrendingUp, Plus, Eye, CheckCircle } from 'lucide-react'
...
export default function DashboardPage() {
  ...
  const statCards = stats ? [
    { label: 'Total Jobs', value: stats.jobs.total, sub: `${stats.jobs.active} active`, icon: Briefcase, color: 'bg-blue-50 text-blue-600', href: '/admin/dashboard/jobs' },
    { label: 'Contacts', value: stats.contacts.total, sub: `${stats.contacts.new} new`, icon: MessageSquare, color: 'bg-emerald-50 text-emerald-600', href: '/admin/dashboard/contacts' },
    { label: 'Applications', value: stats.applications.total, sub: `${stats.applications.pending} pending`, icon: FileText, color: 'bg-amber-50 text-amber-600', href: '/admin/dashboard/applications' },
    { label: 'Testimonials', value: stats.testimonials.total, sub: `${stats.testimonials.active} active`, icon: Star, color: 'bg-purple-50 text-purple-600', href: '/admin/dashboard/testimonials' },
  ] : []
      ...
      <div className="bg-white rounded-xl border border-zinc-200 p-5">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Link href="/admin/dashboard/testimonials" className="inline-flex items-center gap-2 px-4 py-2 border border-zinc-200 text-zinc-700 rounded-lg text-sm font-medium hover:bg-zinc-50 transition-colors">
            <Star className="w-4 h-4" /> Manage Testimonials
          </Link>
        </div>
      </div>
                ...
                <tr key={j.id} className="hover:bg-zinc-50/50 transition-colors">
                  ...
                  <td className="px-5 py-3">
                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${j.isActive ? 'bg-emerald-50 text-emerald-700' : 'bg-zinc-100 text-zinc-500'}`}>
                      {j.isActive ? <CheckCircle className="w-3 h-3" /> : <Clock className="w-3 h-3" />}
