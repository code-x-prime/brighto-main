  ...
  return (
    <section id="services" className="py-16 sm:py-12 lg:py-16 bg-gradient-to-b from-slate-50 to-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Primary Services */}
        <div className="mb-4">
          <p className="text-xs font-bold text-blue-900 uppercase tracking-widest mb-4 sm:mb-6">Primary Services</p>
        ...
        <div>
          <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-4 sm:mb-6 mt-8">Additional Solutions</p>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {ADDITIONAL_SERVICES.map((service, idx) => (
              <Link
                key={idx}
                href={service.href}
                className={`group rounded-xl sm:rounded-2xl border border-slate-200 p-4 sm:p-6 hover:shadow-xl hover:border-blue-300 transition-all cursor-pointer bg-gradient-to-br ${service.gradient}`}
              >
                <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-lg sm:rounded-xl bg-white shadow-sm flex items-center justify-center mb-3 sm:mb-4 group-hover:shadow-md transition-shadow">
                  <Image
                    src={service.icon}
                    alt={service.title}
                    width={26}
